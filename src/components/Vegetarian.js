...
import { addObject, vegetarianRecipes, vegetarianFavorite } from '../Actions/actions';
...
export default function Vegetarian() {
  ...
  const selector = useSelector(state => state.vegetarianRecipes)
  ...
  const getData = async () => {
    await axios.get("https://themealdb.com/api/json/v1/1/filter.php?c=Vegetarian").then((response) => {
      dispatch(vegetarianRecipes(response.data.meals.map(eachRecipe => {
  ...
  return (
    <div className="d-flex flex-column gap-3">
      <h1 className='text-center text-light'>Vegetarian Recipe's</h1>
